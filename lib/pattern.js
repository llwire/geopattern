...
var STROKE_OPACITY   = 0.01;
var OPACITY_MIN      = 0.10;
var OPACITY_MAX      = 0.25;


var DEFAULTS = {
	geoColors: [FILL_COLOR_LIGHT, FILL_COLOR_DARK],
	baseColor: '#933c3c',
	baseShape: 'square',
	width: 100,
};
...
var Pattern = module.exports = function (string, options) {
	...
	this.opts.strokeColor = this.opts.baseColor || STROKE_COLOR
	this.hash = options.hash || sha1(string);
	this.svg = new SVG(this.opts.width);
...
Pattern.prototype.toBase64 = function () {
	...
	if (typeof window !== 'undefined' && typeof window.btoa === 'function') {
		...
		b64 = new Buffer.from(str, 'base64');
	}
...
Pattern.prototype.fillColor = function (val) {
	var idx = val % this.opts.geoColors.length

	return this.opts.geoColors[idx]
}

Pattern.prototype.fillOpacity = function (val) {
	return map(val, 0, 15, OPACITY_MIN, OPACITY_MAX);
}
...
Pattern.prototype.generateBackground = function () {
	...
	var preset = this.opts.baseShape + 'Preset' || 'squarePreset'

	this.svg[preset]({ fill: color.rgb2rgbString(rgb) });
};
...
Pattern.prototype.drawInnerMosaicTile = function (x, y, triangleSize, vals) {
	var triangle = buildRightTriangleShape(triangleSize);
	var opacity  = this.fillOpacity(vals[0]);
	...
	opacity = this.fillOpacity(vals[1]);
...
Pattern.prototype.drawOuterMosaicTile = function(x, y, triangleSize, val) {
	var opacity  = this.fillOpacity(val);
